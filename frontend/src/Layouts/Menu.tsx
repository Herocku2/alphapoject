...
const MenuItemWithChildren = ({
  ...
  return (
    <li className={`nav-item ${open ? 'active' : ''}`}>
      ...
      >
        {item.icon && <i className={item.icon} style={{ marginBottom: '5px', fontSize: '1.3em' }} />}
        <span> {item.label}</span>
        <span className="menu-arrow" style={{ position: 'absolute', right: '10px', top: '50%', transform: 'translateY(-50%)' }} />
      </Link>
      <Collapse in={open}>
        <div>
...
const MenuItemLink = ({ item, className }: SubMenus) => {
  return !item.isExternal ? (
    <Link
      ...
      style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center' }}
    >
      {item.icon && <i className={item.icon} style={{ marginBottom: '5px', fontSize: '1.8em' }} />}
      <span> {item.label}</span>
    </Link>
  ) : (
